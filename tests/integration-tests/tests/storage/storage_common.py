...
def test_efs_correctly_mounted(remote_command_executor, mount_dir, tls=False, iam=False, access_point_id=None):
    ...
    if access_point_id:
        if iam:  
            assert_that(result.stdout).matches(rf".* {mount_dir} efs _netdev,noresvport,tls,iam,accesspoint={access_point_id} 0 0")
        else: 
            assert_that(result.stdout).matches(rf".* {mount_dir} efs _netdev,noresvport,tls,accesspoint={access_point_id} 0 0")
    else: #tls is always enabled
